...
<TriggerFunctionSyncProvider>
  {/* Tous les exemples TriggerFunction bénéficient de la synchronisation d'id */}
...
```
  ...
  <TriggerFunction method="DELETE" url={`${TRIGGER_API_BASE_URL}/widgets/:id`} label="Supprimer le widget" />


## 8bis. Ajouter un endpoint POST /widgets pour la création groupée (bulk)

Il est tout à fait RESTful d’exposer un endpoint permettant de créer plusieurs ressources d’un coup, tant que le groupe est bien identifié et manipulé de façon sûre. Cela permet d’optimiser les coûts et les performances (moins d’appels réseau, moins de latence, etc.).

Voici comment adapter l’implémentation de l’étape 4 pour supporter la création groupée :
...
<TriggerFunction
  method="POST"
  url={`${TRIGGER_API_BASE_URL}/widgets`}
  body={[{ weight: 10, color: 'red' }, { weight: 20, color: 'blue' }]}
  label="Créer plusieurs widgets (bulk)"
/>

:::tip
Le batch n’est pas réservé à la création : on peut aussi imaginer PATCH ou DELETE groupés, tant que l’opération reste sûre et bien documentée.
:::

## 8ter. Ajouter un endpoint DELETE /widgets pour tout supprimer (purge)

Pour le tutoriel, il peut être utile de pouvoir réinitialiser la collection de widgets d’un coup. Attention, ce genre d’opération est puissante et doit être réservée à des cas bien identifiés (admin, démo, etc.).

```typescript
router.delete('/widgets', (req, res) => {
  // highlight-start
  // Ici, on supprime tous les widgets d’un coup
  // highlight-end
